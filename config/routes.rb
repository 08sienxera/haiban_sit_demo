Rails.application.routes.draw do
  ...
  namespace :manager do
    ...
    resources :home, only: [] do
      ...
      collection do
        get :dev, :menu, :error, :destroy_job_err, :ck_job, :mk_test_vacation, :mk_test_lunch_order,
            :test_calc_work_time
      end
    end
    # マスタ関連
    ...
    resources :wh_calendars, only: %i[index update show] do
      collection do
        get :test
      end
    end
    resources :boards
    resources :o_boards, only: %i[index update show] do
      member do
        get :file_output
      end
    end
    ...
    resources :board_comments, only: %i[create update]
    resources :board_group_wokers, only: %i[edit update show]
    resources :lunch_orders, only: %i[index create edit update]
    ...
    resources :cargos, only: %i[index create update]
    resources :wk_assignment, :result_assignment, only: %i[index create update] do
      collection do
  ...
  namespace :bos do
    ...
    resources :cargos, only: %i[index create update]
    resources :print_files, only: [:create]
    resources :lunch_order, only: %i[edit update] do
      collection do
        get :history
      end
    end
  ...
  namespace :operator do
    ...
    resources :boards, only: %i[index update show] do
      member do
        get :file_output
      end
    end
  ...
  # 開発用デモメニュー
  resources :dev_menu, only: %i[index create]
  if Rails.env.development? || Rails.env.test?
    root to: 'dev_menu#index'
  else
    root to: 'login#login'
  end
  get '*path' => 'application#render_404' unless Rails.application.config.consider_all_requests_local
end
