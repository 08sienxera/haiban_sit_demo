class Manager::BoardCommentsController < Manager::HomeController
  ...
  def create
    ...
    if @board.present?
      begin
        ...
        redirect_to :controller=>:boards,:action=>:edit,:id=>params[:board_id]
      rescue => e
  ...
  def update
    begin
      if params[:board_comment_body].present?
        ...
        redirect_to :controller=>:boards,:action=>:edit,:id=>@comment.board.id
      else
